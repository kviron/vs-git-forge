...
const MAX_COMMITS_ENTRIES = 500;

/**
 * Единый API для webview и расширения: запросы данных (getBranches, getCommits и т.д.)
 * и хелперы вроде получения ветки из .git/HEAD.
 * Создаётся один экземпляр в extension и передаётся в panelProvider и messageHandler.
 */
export class GitForgeApi {
  /**
   * Читает имя ветки из .git/HEAD (без встроенного Git-расширения).
   */
  async getBranchFromGitHead(): Promise<string | undefined> {
   ...
   * - getIdeContext — контекст IDE (язык, тема, приложение)
   * - initRepo — инициализация репо (params: rootUri?)
   * - showCreateBranchDialog, showCreateTagDialog, pullBranch
   * - getCurrentBranch, getLocalBranches, getRemoteBranches, getTags, getBranches
   * - getCommits (params: ref?, maxEntries?), getChangedFiles, getCommitChangedFiles (params: commitHash), getRepositoryRoot
   */
  async request(
    method: string,
    ...
    if (method === "getIdeContext") {
      return { data: getIdeContext() };
    }
    ...
    if (!repo) {
      const branch = await this.getBranchFromGitHead();
    ...
    try {
      switch (method) {
        case "getCurrentBranch": {
          ...
          const short = name ? toShortBranchName(name) || null : null;
          return { data: short };
        }
        ...
        case "getBranches": {
          ...
          const currentShort = headName ? toShortBranchName(headName) || null : null;
      ...
      log.errorException(e, `GitForgeApi.request(${method})`);
