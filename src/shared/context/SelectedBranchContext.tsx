...
export function SelectedBranchProvider(props: { children: JSX.Element }) {
  ...
  const setSelectedBranch: Setter<Branch | null> = (value) => {
    ...
    const next: Branch | null = typeof value === 'function' ? value(selectedBranch()) : value;
    setSelectedBranchRaw(next);
  };
  const setSelectedTag: Setter<Tag | null> = (value) => {
    setSelectedBranchRaw(null);
    const next: Tag | null = typeof value === 'function' ? value(selectedTag()) : value;
    setSelectedTagRaw(next);
  };
  return (
